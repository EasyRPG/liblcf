...
RPG::MapInfo LMT_Reader::ReadMapInfo(Reader& stream) {
	...
	while (!stream.Eof()) {
		...
		case ChunkMapInfo::encounter_steps:
			mapinfo.encounter_steps = stream.ReadInt();
			break;
		case ChunkMapInfo::area_rect:
			mapinfo.area_x = stream.Read32();
			mapinfo.area_y = stream.Read32();
			mapinfo.area_w = stream.Read32() - mapinfo.area_x;
			mapinfo.area_h = stream.Read32() - mapinfo.area_y;
			break;
		default:
